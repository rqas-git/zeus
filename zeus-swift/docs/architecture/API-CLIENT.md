...
5. Session IDs are sequential integers and are carried as numbers in JSON
   bodies, responses, and SSE events.
6. Turn and session event streams are read as raw SSE bytes, preserving blank
   event-separator lines before decoding payloads into `AgentServerEvent`.
7. HTTP non-2xx responses become `AgentClientError.httpStatus`.
8. Empty or unparsable streams include a short preview in the thrown error.
