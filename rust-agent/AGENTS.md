...
dedicated `git_status`, `git_diff`, `git_log`, `git_query`, `git_add`,
`git_restore`, and `git_commit` wrappers. `exec_command` runs shell commands
from the workspace and does not currently apply command-level allow/deny
protections; those restrictions are intentionally deferred and must be
implemented before using `workspace-exec` for untrusted sessions. The git
wrappers remain available for structured repository operations. When stdout or
