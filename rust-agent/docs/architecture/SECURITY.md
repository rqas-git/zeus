...
workspace search behavior. Revisit the exception when FFF offers a release that
removes the `bincode 1.x` dependency, when a maintained FFF fork is available,
or when replacing FFF no longer risks search quality regressions.
