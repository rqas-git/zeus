...
into client, context-window, and output configuration.

## Flow

1. `main` calls `AppConfig::from_env` once at startup.
2. `ClientConfig` configures model transport.
3. `ContextWindowConfig` configures prompt history bounds.
4. `OutputConfig` configures terminal delta buffering.
5. The resulting values are passed into long-lived services.
