...
- `tools::tests::benchmark_read_file_capped_large_file`
  ...
  read path.
- `tools::tests::benchmark_workspace_search_current_repo`
  Measures cold workspace search snapshot initialization plus warm fuzzy path
  search and content search against the current repository.
